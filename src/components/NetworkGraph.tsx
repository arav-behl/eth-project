...
export function NetworkGraph({ nodes, edges, onNodeClick, onEdgeSelect, selectedEdgeKey }: Props) {
              ...
              return (
                <g key={`e-${i}`}>
                  ...
                  <line
                    ...
                    onClick={() => {
                      if (hasRelationship) onEdgeSelect(edge);
                    }}
                  />
                  ...
                  {/* Edge label only on hover/selection, otherwise the canvas gets noisy fast */}
                  {hasRelationship && (isEdgeHovered || isSelected) && (
                    <g
                      transform={`translate(${midX},${midY})`}
                      opacity={hoveredNode ? (edgeConnected ? 1 : 0.06) : isSelected || isEdgeHovered ? 1 : 0.75}
